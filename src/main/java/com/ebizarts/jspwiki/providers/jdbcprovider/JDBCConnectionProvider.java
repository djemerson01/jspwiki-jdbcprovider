...
public class JDBCConnectionProvider extends ConnectionProvider
{
    ...
    public void initialize(WikiEngine engine, final Properties config) throws NoRequiredPropertyException
    {
        ...
        driver = TextUtil.getRequiredProperty(config, PROP_PREFIX + ".driverClassName");
        url = TextUtil.getRequiredProperty(config, PROP_PREFIX + ".url");

        connectionProperties = parseAdditionalProperties(config, DRIVER_PROP_PREFIX);
        connectionProperties.put("user", TextUtil.getRequiredProperty(config, PROP_PREFIX + ".username"));
        connectionProperties.put("password", TextUtil.getRequiredProperty(config, PROP_PREFIX + ".password"));
